	...
	initNPC() {
		...
		npcs.forEach(npc => {
			...
			if(npc.texture !== 'stand' && npc.texture !== 'statue'){
				this.matter.add.sprite(npc.x, npc.y - 20 ,"bubble").setStatic(true).setSensor(false)
			} else {
				this.matter.add.sprite(npc.x, npc.y - 40 ,"bubble").setStatic(true).setSensor(true)
			}
			...
			var compoundBody = Body.create({
				parts: [
					Bodies.circle(this[npc.texture].x, this[npc.texture].y, 12, { isSensor: false, label: colliderName }),
					Bodies.circle(this[npc.texture].x, this[npc.texture].y, 24, { isSensor: true, label: sensorName }),
				],
				frictionAir: 0.35,
			})
