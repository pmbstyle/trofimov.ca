...
const editorOptions = {
    debug: 'info',
    modules: {
        toolbar: [{ 'font': [] }, { 'align': [] }, 'bold', 'italic', 'underline', 'strike', 'blockquote', 'code-block',{ 'color': [] }, { 'background': [] },]
    },
...
<template>
	<div id="mailform" class="flex-1 flex flex-col">
        <div class="toast toast-middle toast-center">
            <div class="alert alert-warning w-[300px] animate-bounce text-center" v-if="error.status">
                <span>{{error.message}}</span>
            </div>
            <div class="alert alert-success w-[300px] animate-bounce text-center" v-if="sent">
                <span>Message sent. Thank you.</span>
            </div>
        </div>
        <div class="flex items-center pt-4">
            <span class="w-[100px] text-right">From:</span>
            <input type="text" class="input input-bordered w-full ml-4" id="from" name="from" v-model="message.from" placeholder="Your email" />
        </div>
        <div class="flex items-center pt-2">
            <span class="w-[100px] text-right">To:</span>
            <input type="text" class="input input-bordered w-full ml-4" id="to" name="to" value="slava@trofimov.ca" @keyup="handleToChange"/>
        </div>
        <div class="flex items-center pt-2">
            <span class="w-[100px] text-right">Subject:</span>
            <input type="text" class="input input-bordered w-full ml-4" id="subject" name="subject" v-model="message.subject"/>
        </div>
        <div class="pt-2 pb-4 flex-1 flex flex-col">
            <QuillEditor :options="editorOptions" v-model:content="message.text" contentType="text"/>
        </div>
        <div class="btn-row flex items-center">
            <button class="btn btn-primary" @click="validateInput" :disabled="error.status || sent">Send</button>
        </div>
    </div>
</template>